...
Material materialNew(MaterialType type, Color cd, Color cs, Color cr){
    ...
    if(type == MatteMaterial){
        ...
        res.materialData.m.ambientBRDF = ambient, res.materialData.m.diffuseBRDF = diffuse;
        P_BRDF(res.materialData.m.ambientBRDF); P_BRDF(res.materialData.m.diffuseBRDF);
        //PMatte(res);
        return res;
    }
}
...
*       -> lambertian cd
            -> R, G, B
*/
#define N_ATTRIBUTES 5

double materials[N_MATERIALS*N_ATTRIBUTES];

/*
    Adiciona material no array global de materiais pela id;

    id -> número do material
    kd -> coeficiente difuso
    ka -> coeficiente difuso ambiente
    r,g,b -> cor do material

*/
void addMaterial(int id, double kd, double ka, double r, double g, double b){

    materials[id*N_ATTRIBUTES]   = ka;
    materials[id*N_ATTRIBUTES+1] = kd;
    materials[id*N_ATTRIBUTES+2] = r;
    materials[id*N_ATTRIBUTES+3] = g;
    materials[id*N_ATTRIBUTES+4] = b;
