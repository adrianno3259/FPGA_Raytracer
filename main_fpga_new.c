...
typedef struct{
    int nx, ny, nz;
    BBox bbox;
    Triangle grid[MAX_TRIANGLES/4][MAX_TRIANGLES];
} Grid;


Vec3d gridMinPoint(Grid g){
...
int numTriangles, numMaterials, numLights;
...
int main(){
    ...
    const int IMAGE_VRES = 500, IMAGE_HRES = 500;
    int i, j;
    ImagePPM im = imageNew(IMAGE_HRES, IMAGE_VRES);
    Camera cam = cameraNew(IMAGE_HRES, IMAGE_VRES, vecNew(60, 0, 60),
                           vecNew(0,0,0), vecNew(0,0,1), 200, 1.0);
